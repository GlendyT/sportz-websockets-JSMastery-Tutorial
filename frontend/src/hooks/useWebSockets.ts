...
export const useWebSocket = (
  ...
  const initConnection = useCallback(() => {
    ...
    try {
      ...
      socket.onerror = () => {
        // WebSocket error events are generic in browsers and dont contan descriptive messages.
      ...
      socket.onclose = (event) => {
        if (!isIntentionalClose.current) {
          ...
          // if closed intentioally, just set status
          setStatus("disconnected");
        }
      };
    } catch (e) {
  ...
  const subscribeMatch = useCallback(
    (matchId: string | number) => {
      const normalize = normalizeId(matchId);
      subscribedMatchIdsRef.current.add(normalize);
      sendMessage({ type: "subscribed", matchId });
    },
    [sendMessage],
  );
  ...
  const unsubscribeMatch = useCallback(
    (matchId: string | number) => {
      const normalize = normalizeId(matchId);
      subscribedMatchIdsRef.current.delete(normalize);
      sendMessage({ type: "subscribed", matchId });
    },
    [sendMessage],
  );
